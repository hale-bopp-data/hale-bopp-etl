...
from etl_hale_bopp.watcher import _process_events


def test_process_trigger_event(tmp_path, monkeypatch):
    monkeypatch.setattr("etl_hale_bopp.watcher.EVENTS_DIR", tmp_path)

    event = {"event_id": "e1", "event_type": "db.schema.deploy.completed", "payload": {}}
    (tmp_path / "e1.json").write_text(json.dumps(event))

    runs = []
    monkeypatch.setattr("etl_hale_bopp.watcher.run_by_id", lambda pid, cfg=None: runs.append(pid))
